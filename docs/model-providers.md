...
正式默认是 `openai_compatible`，但 `api_base`、`model`、`api_key` 全部留空，必须手动填写。配置缺失时后端拒绝开始面试，不再静默 fallback 到 mock。

真实 LLM 调用过程中如果服务商报错、网络超时或返回空内容，默认直接中断当前轮次并返回错误，避免用户误以为仍在使用真实模型练习。只有在前端手动开启“LLM 失败时允许 Mock 回退（仅开发调试）”时，后端才会把失败轮次降级到 deterministic mock 逻辑。
