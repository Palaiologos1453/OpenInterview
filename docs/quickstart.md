...
测试失败时看状态栏里的“建议”。常见原因是 Key 错、模型名不存在、Base URL 少了 `/v1` 或兼容模式路径、账号额度不足、网络超时。真实 LLM 调用失败或返回空内容时默认不会回退到 Mock；“LLM 失败时允许 Mock 回退”只建议开发调试时开启。
...
顶部的 `题库覆盖` 可以查看当前方向各主题题量和追问/评分点覆盖。Java 后端核心 topic 使用更高覆盖阈值，AI 应用方向和补充 topic 会按当前题库阶段使用轻量阈值。`错题本` 用来集中复练报告里的低分题和缺失点。

当前题库已从 JavaGuide 的 Java 后端、数据库、缓存、分布式、高可用、高性能、系统设计和 AI 应用目录批量改写生成。更新本机 `D:\JavaGuide` 后，可以重新生成：
...
历史 JSON 会包含报告缓存、回答记录、转写、trace 和错题本。导入只写入本机 SQLite，同 ID 记录会以导入文件为准。
