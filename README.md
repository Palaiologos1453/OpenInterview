...
OpenInterview 是一个面向 Java 后端校招的本地 AI 模拟面试工具。
...
当前公开入口先聚焦 Java 后端。前端、测开、数据、算法、SRE、嵌入式等方向没有在目录里开放，避免题库深度不足时给用户错误预期；后续等题库质量跟上再恢复多方向入口。
...
2. 在左侧“模型配置（本机保存）”里填写 LLM：
   ...
   - API Key：你的 key
3. 点击“测试 LLM”，确认模型能返回。
4. 选择难度、模式和面试官风格，开始面试。方向当前固定为 Java 后端。
...
- `八股连环追问型`：围绕 Java 后端高频知识点连续追问原理、边界和误区。
